...
> 💫 **Wild Rift の最新情報をお届け！**
>
> 毎日、自動で最新ニュースをチェックして RSS フィードに変換するよ〜！
...
- 🎮 Wild Rift の公式ニュースを 1 日 1 回自動でチェック
...
これで設定完了！GitHub Actions が 1 日 1 回（午前）自動的に実行されて、最新ニュースをチェックするよ〜✨

### GitLab で設定する場合

1. このリポジトリをフォーク
2. [新しいパイプラインスケジュールを作成](../../pipeline_schedules/new)（Build ˃ Pipeline schedules）
3. 実行時間を設定（例：`30 20 * * *` で 5:30(JST) に実行）
